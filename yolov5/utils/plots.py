...
class Annotator:
    # Simple annotator class to draw boxes and labels on images
    ...
    def box_label(self, box, label='', color=(255, 0, 0), txt_color=(255, 255, 255)):
        # Draw one bounding box with label
        p1, p2 = (int(box[0]), int(box[1])), (int(box[2]), int(box[3]))
        cv2.rectangle(self.im, p1, p2, color, thickness=self.lw, lineType=cv2.LINE_AA)
        if label:
            tf = max(self.lw - 1, 1)  # font thickness
            w, h = cv2.getTextSize(label, self.font, fontScale=self.fs / 10, thickness=tf)[0]  # text width, height
            outside = p1[1] - h >= 3
            p2 = p1[0] + w, p1[1] - h - 3 if outside else p1[1] + h + 3
            cv2.rectangle(self.im, p1, p2, color, -1, cv2.LINE_AA)  # filled
            cv2.putText(self.im, label, (p1[0], p1[1] - 2 if outside else p1[1] + h + 2),
                        self.font, self.fs / 10, txt_color, thickness=tf, lineType=cv2.LINE_AA)
